...
class RefinedFusionAttention(nn.Module):
    ...
    def get_position_weights(self, rel_pos, temperature):
        pos_weights = self.pos_mlp(rel_pos)  # (h*w, h*w, heads)
        dist = rel_pos[..., -1]
        attn_mask = torch.exp(-dist / temperature).unsqueeze(2)  # 距离越大，权重越小
        return pos_weights * attn_mask
    ...
    def forward(self, x):
        ...
        # pos_weights = self.pos_mlp(rel_pos)  # (h*w, h*w, heads)
        pos_weights = self.get_position_weights(rel_pos,self.temperature)
        pos_weights = pos_weights.permute(2, 0, 1).unsqueeze(0)  # (1, heads, h*w, h*w)

        # x_sim = self.sim_mlp(x)
        # 4. 计算特征相似度权重
        ...
        # feature_sim = feature_sim.view(b * l, h * w, h * w, self.num_heads)
        # feature_sim = feature_sim.permute(0, 3, 1, 2)
