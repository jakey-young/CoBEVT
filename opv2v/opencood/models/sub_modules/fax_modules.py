...
#
#         # flattening
#         q = rearrange(q, 'b n x y w1 w2 d -> b (x y) (n w1 w2) d') # i=0:(b,64,1024,128) (b,16,256,128)把n和w放在一起展平，体现从多个摄像头视角进行注意力计算
...
#         self.version2 = nn.Conv2d(256,128,kernel_size=1)
...
#
# # -----------------------------------------------------版本2--------------------------------------------------------------
# #         if bev_pre is not None:
# #             query_pre = F.interpolate(bev_pre, size=(query.size(-1), query.size(-1)), mode='bilinear',
# #                                       align_corners=False)
# #             query = rearrange(query, 'b n ... -> (b n) ...')
# #             fusion = torch.cat([query,query_pre],dim=1)
# #             Conv2d = self.version2
# #             query = Conv2d(fusion)
# #             query = rearrange(query, '(b n) ... -> b n ...',n=1)
# #         else: query_pre = None
# # ----------------------------------------------------------------------------------------------------------------------
#
# # -----------------------------------------------------版本3--------------------------------------------------------------
...
# #                                       align_corners=False)
# #             query = rearrange(query, 'b n ... -> (b n) ...')
# #             fusion = query_pre + query
# #             query = rearrange(fusion, '(b n) ... -> b n ...',n=1)
# #         else: query_pre = None
# # ----------------------------------------------------------------------------------------------------------------------
...
# #             scale_channel = torch.nn.Conv2d(concat.size(1),query.size(-1),kernel_size=1).to(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
